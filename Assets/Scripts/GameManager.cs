...
public class GameManager : MonoBehaviour
{
    ...
    void Update()
    {
        ...
        if (sGameStarted && !mGameLost)
        {
            ...
            mCurrentScore += Time.deltaTime * 500;
            // Celebrations on few important values
            if (mCurrentScore > 25000.0f && celebration25)
            {
                FindObjectOfType<AudioManager>().Play("Celeb25");
                celebration25 = false;
            }
            if (mCurrentScore > 50000.0f && celebration50)
            {
                FindObjectOfType<AudioManager>().Play("Celeb50");
                celebration50 = false;
            }
            if (mCurrentScore > 100000.0f && celebration100)
            {
            ...
            if (mCurrentScore > allTimeHigh)
            {
                allTimeHigh = mCurrentScore;
                GetChildNamed(scoreText, "ATHValue").GetComponent<Text>().text = $"{(int)(mCurrentScore)}$";
            }
        }
    }
    ...
    public void SetupGame()
    {
        ...
        { // Setup already started game -> Retry.
            ...
            GetChildNamed(scoreText, "ATHValue").GetComponent<Text>().text = $"{(int)(allTimeHigh)}$";
        }
        else
